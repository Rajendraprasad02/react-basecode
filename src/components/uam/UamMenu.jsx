...
export default function UamMenu() {
  ...
  useEffect(
    () => async () => {
      const menu = await unProtectedCall("/api/menu-creator");
      const newActions = await unProtectedCall("/api/actions");
      setUam(menu?.menus);
  ...
  const handleMenuSubmit = async () => {
    try {
      ...
      await unProtectedCall("/api/menu-creator", transformed, "post");
  ...
  const handleClick = useMemo(
    () => (event, module) => {
      ...
      const scrollOffsetLeft = window.scrollX;

      setAnchorPosition({
        top: top + scrollOffsetTop + 5, // Adjusting for scroll and adding padding
        left: left + scrollOffsetLeft + 5,
      });
